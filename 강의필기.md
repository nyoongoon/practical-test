...
## 부록 - 학습테스트 (라고 하기엔 중요한 것들)
- 학습테스트 : 잘 모르는 기능, 라이브러리, 프레임워크를 학습하기 위해 작성하는 테스트 
- 팀 내 공유용으로 learning 패키지를 두어 학습 테스트를 남길 수도 있다.

# Spring REST Docs
- 테스트 코드를 통한 API문서 자동화 도구 
- 기본적으로 AsciiDoc을 사용하여 문서를 작성한다.
- cf) jar파일 실행 후 문서 들어가짐
- 프로젝트 시작할 때 명세를 정하고 나서 Mock API 같은 문서를 먼저 만들어서 공유를 하는 편 -> 
- docs test 먼저 작성 (빈밸리데이션까지) -> 배포 -> 서비스단 개발 진행 